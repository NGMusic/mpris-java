...
interface TrackList: DBusInterface {
    ...
    class TrackListReplaced(path: DBusPath,
                            tracks: List<TrackId>,
                            currentTrack: TrackId): DBusSignal(path, tracks, currentTrack)
    ...
    class TrackAdded(path: DBusPath,
                     metadata: Map<String, Variant<*>>,
                     afterTrack: TrackId): DBusSignal(path, metadata, afterTrack)

    /** Indicates that a track has been removed from the track list.
     *
     * @param path The path to the object this is emitted from.
     * @param trackId The identifier of the track being removed. */
    class TrackRemoved(path: DBusPath,
                       trackId: TrackId): DBusSignal(path, trackId)
    ...
    class TrackMetadataChanged(path: DBusPath,
                               trackId: TrackId,
    ...
    fun AddTrack(uri: String, afterTrack: TrackId, setAsCurrent: Boolean)
